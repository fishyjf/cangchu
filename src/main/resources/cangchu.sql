...
CREATE TABLE `t_inventory` (
                               ...
                               `status` VARCHAR ( 1 ) DEFAULT NULL COMMENT '状态 0：库存内没有；1：库存内存在该商品',
                               PRIMARY KEY ( `Id` ),
                               UNIQUE KEY `uniq_inventory_goods_id` ( `goods_id` ) USING BTREE
) ENGINE = INNODB AUTO_INCREMENT = 1 DEFAULT CHARSET = utf8 COMMENT = '库存信息表';
...
CREATE TABLE `t_goods` (
                           ...
                           PRIMARY KEY ( `id` ),
                           UNIQUE KEY `uniq_goods_name` ( `name` ) USING BTREE
) ENGINE = INNODB AUTO_INCREMENT = 1 DEFAULT CHARSET = utf8 COMMENT = '商品信息';

DROP TABLE IF EXISTS `t_into_out_info`;
CREATE TABLE `t_into_out_info` (
  `Id` int(11) NOT NULL AUTO_INCREMENT,
  `goods_id` int(11) NOT NULL COMMENT '商品ID',
  `number` int(11) DEFAULT 1 COMMENT '出入库数量',
  `type` varchar(1) DEFAULT NULL COMMENT '类型 0：出库；1：入库',
  `create_time` timestamp DEFAULT CURRENT_TIMESTAMP COMMENT '出入库时间',
  PRIMARY KEY (`Id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='出入库信息表';
